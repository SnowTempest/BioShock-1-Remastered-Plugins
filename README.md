...
- [❄️ Plugin Loader - DXGI.DLL](#️-plugin-loader---dxgidll)
...
## ❄️ Plugin Loader - DXGI.DLL

`dxgi.dll` is the plugin loader. This DLL file acts as a fake proxy that allows us to load additional plugins into the game automatically by pretending to be the real dxgi.dll the game expects to be loaded.

On launch, the file will:
- Forward all original DXGI calls to the real DLL file the game expects.
- Scans the game directory at "SteamLibrary\steamapps\common\BioShock Remastered\Build\Final", looks for a folder named `BioPlugins`, then locates all DLL plugin files within that folder.
...
4. Paste the contents within "SteamLibrary\steamapps\common\BioShock Remastered\Build\Final".
...
```
\SteamLibrary\steamapps\common\BioShock Remastered\Build\Final\
├── BioshockHD.exe
├── dxgi.dll        ← plugin loader
└── BioPlugins/
    ├── loader.log  ← generated on launch
    └── AnotherPlugin.dll
```

## ❄️ Plugin Loader Conflicts

If another plugin or mod you are using also ships as a proxy DLL (e.g. their own dxgi.dll), it will conflict with this loader as only one proxy can occupy that filename at a time.
...
BioShock 1 Remastered Plugins is an unofficial, non-commercial, community plugin repository. All original game content, including assets, characters, story, and company names, are the property of 2K Games.

These plugins are intended for players who already own a legitimate copy of BioShock Remastered. It is not an official download, patch, or product from 2K Games.
